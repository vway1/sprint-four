...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runCalories := ((runningCaloriesMeanSpeedMultiplier * speedInKmH * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH)
	return runCalories
}
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	speedInMsec := math.Pow((meanSpeed(action, duration) * kmhInMsec), 2)
	heightInM := height / cmInM
	walkCalories := ((walkingCaloriesWeightMultiplier*weight + (speedInMsec/heightInM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return walkCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	speedSwimInKmH := swimmingMeanSpeed(lengthPool, countPool, duration)
	swimCalories := (speedSwimInKmH + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return swimCalories
}
